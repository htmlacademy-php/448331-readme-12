...
$sql_content_types_select = "SELECT type AS post_type, class AS post_class
                             FROM content_type;";
...
if (count($errors)) {      // если были ошибки, загружаем страницу и показываем ошибки
  ...
  $page_content = include_template('post_add.php', ['content_type' => $content_types, 'errors' => $errors]);
  print($page_content);
...
$page_content = include_template('post_add.php', ['content_type' => $content_types]);
print($page_content);
?>